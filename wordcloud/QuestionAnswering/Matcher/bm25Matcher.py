...
class bestMatchingMatcher(Matcher):
    ...
    def initBM25(self):
        ...
        self.avgdl = sum([float(len(title)) for title in self.segTitles]) / self.D

        for seg_title in self.segTitles:

            #計算所有分詞的數量，製成{word:count}的dict object
            tmp = {word:seg_title.count(word) for word in seg_title} 
            self.f.append(tmp)

            #df[word] = cnt ==> 在所有文件中有cnt個文件含word分詞
            for word, cnt in tmp.items():
